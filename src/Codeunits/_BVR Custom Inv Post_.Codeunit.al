codeunit 50142 "BVR Custom Inv Post"
{
    ...
    begin
        ...
        repeat
            ...
            if DimSetId = 0 then
                DimSetId := InvHdr."Dimension Set ID";
            if InvLine.Type = InvLine.Type::"G/L Account" then begin
                // G/L Account lines carry no receipt accrual: combine them by account and
                // post one entry per account directly to the account entered on the line (block 4 below).
                InvLine.TestField("No.");
                if GLAccAmounts.ContainsKey(InvLine."No.") then
                    GLAccAmounts.Set(InvLine."No.", GLAccAmounts.Get(InvLine."No.") + Round(InvLine."Line Amount", 0.01))
                else
                    GLAccAmounts.Add(InvLine."No.", Round(InvLine."Line Amount", 0.01));
                GLTotal += Round(InvLine."Line Amount", 0.01);
            end else begin
        ...
        // Sales/Use tax calculation by Tax Area Code + Tax Group Code (all line types)
        ...
        if (InvoiceBaseAmt = 0) and (GLTotal = 0) and (TaxAmt = 0) then
            Error('Amount is zero, please check the values for Quantity and Cost');

        DiffBase := Round(InvoiceBaseAmt - ReceiptBaseAmt, 0.01);
        TotalInvoiceAmt := Round(InvoiceBaseAmt + GLTotal + TaxAmt, 0.01);
        ...
        // 4) Post combined G/L Account lines: one entry per G/L account, balanced to payables
        //    Debit G/L Account (combined), Credit Payables
        foreach GLAcc in GLAccAmounts.Keys() do begin
            GLAmt := Round(GLAccAmounts.Get(GLAcc), 0.01);
            if GLAmt <> 0 then begin
                ...
                GenJnlLine.Validate("External Document No.", InvHdr."Vendor Invoice No.");
                GenJnlLine.Validate("Account Type", GenJnlLine."Account Type"::"G/L Account");
                GenJnlLine.Validate("Account No.", GLAcc);
                GenJnlLine.Validate(Amount, GLAmt);
                GenJnlLine.Validate("Bal. Account Type", GenJnlLine."Bal. Account Type"::"G/L Account");
                GenJnlLine.Validate("Bal. Account No.", PayablesAcc);
                GenJnlLine."Dimension Set ID" := InvHdr."Dimension Set ID";
                GenJnlPostLine.RunWithCheck(GenJnlLine);
        ...
        InvHdr.Modify(true);
        Message('Custom Invoice posted. Invoice: %1, Posted Invoice: %2. Total=%3 (Base=%4, G/L=%5, Tax=%6)', InvHdr."No.", PostedInvHdr."No.", TotalInvoiceAmt, InvoiceBaseAmt, GLTotal, TaxAmt);
    end;
}
