table 50157 "BVR Sales CrMemo Batch"
{
    // Batch of sales credit memos for AP/AR review, in the spirit of a General Journal Batch.
    //
    // One table per batch process. A batch code therefore means exactly one thing - a sales credit memo batch - and
    ...
    LookupPageId = "BVR Sales CrMemo Batch List";
    ...
    fields
    {
        ...
        field(10; "No. of Sales Cr. Memos"; Integer)
        {
            Caption = 'No. of Sales Credit Memos';
            Editable = false;
            FieldClass = FlowField;
            CalcFormula = count("Sales Header" where("Document Type" = const("Credit Memo"), "BVR Doc Batch No." = field("Code")));
        }
        field(11; "No. of Posted Sales Cr.Memo"; Integer)
        {
    ...
    begin
        if Status = Status::Closed then
            exit(false);
        if not BatchDocMgt.SalesBatchIsEmpty(Enum::"Sales Document Type"::"Credit Memo", "Code") then
            exit(false);
    ...
    var
        BatchDocMgt: Codeunit "BVR Batch Doc Mgt";
    begin
        exit(BatchDocMgt.SalesDocTotal(Enum::"Sales Document Type"::"Credit Memo", "Code"));
    end;
}
